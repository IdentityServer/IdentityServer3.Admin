...
namespace IdentityAdmin.Extensions
{
    public static class PropertyInfoExtensions
    {
        ...
        public static string[] GetEnumValues(this PropertyInfo property)
        {
            if (property == null) throw new ArgumentNullException("property");
            var returnValue = new List<string>();
            if (property.PropertyType.IsEnum)
            {
                MemberInfo[] memberInfos = property.PropertyType.GetMembers().Where(type => type.MemberType ==MemberTypes.Field ).ToArray();
                for (int i = 0; i < memberInfos.Length; i++)
                {
                    var name = memberInfos[i].Name;
                    if (name != "value__")
                    {
                        returnValue.Add(memberInfos[i].Name);
                    }
                }
            }
        ...
        public static PropertyDataType GetPropertyDataType(this Type type)
        {
            ...
            if (type.IsInterface)
            {
                return PropertyDataType.Enumerable;
            }
