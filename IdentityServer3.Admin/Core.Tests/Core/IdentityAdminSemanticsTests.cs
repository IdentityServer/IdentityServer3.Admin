...
{
    public abstract class IdentityAdminSemanticsTests
    {
        abstract protected IIdentityAdminService CreateIdentityAdmin();
        abstract protected bool ValidatePassword(string uid, string pwd);

        readonly IIdentityAdminService subject;

        public IdentityAdminSemanticsTests()
        {
            subject = CreateIdentityAdmin();
        }
        ...
        //}
        [Fact]
        public void DeleteClientAsync_NoSubject_ReturnsError()
        {
            var result = subject.DeleteClientAsync("").Result;
